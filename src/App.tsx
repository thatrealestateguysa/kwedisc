...
const Input = React.forwardRef<HTMLInputElement, React.InputHTMLAttributes<HTMLInputElement>>((props, ref)=> (<input ref={ref} {...props} className={`input ${props.className||''}`} />)); Input.displayName='Input';
...
export default function App(){
  ...
  const [answers,setAnswers]=useState<{most:Trait|null;least:Trait|null}[]>(Array(20).fill(null).map(()=>({most:null,least:null})));
  ...
  const scores=useMemo(()=>{const natural:Record<Trait,number>={D:0,I:0,S:0,C:0};const adaptive:Record<Trait,number>={D:0,I:0,S:0,C:0};answers.forEach(a=>{if(a.most)natural[a.most]+=1;if(a.least)adaptive[{D:'S',I:'C',S:'D',C:'I'}[a.least!] as Trait]+=1});return{natural,adaptive}},[answers]);
  ...
  // PDF (multi-page raster with high scale)
  ...
  const downloadPDF = async ()=>{
    const { pdf } = await buildPdfAndDataUrl();
    pdf.save(`${APP_NAME}_${info.firstName}_${info.lastName}.pdf`);
  };

  // Email using EmailJS sendForm (supports attachments via <input type="file">)
  const emailPDF = async ()=>{
    ...
    try{
      ...
      // Programmatically put File into hidden input
      const dt = new DataTransfer();
      dt.items.add(file);
      if (fileRef.current) fileRef.current.files = dt.files;

      // Fill other hidden fields
      ...
      print(e)
      alert('Email failed. Verify EmailJS keys/service/template & that your template has fields for to_email, cc, from_name, reply_to and an attachment input named report_pdf.')
    finally:
      setSending(false)
  };
  ...
  return (
    <div className="min-h-screen bg-[#0b0b0d] text-neutral-100">
      ...
      <main className="max-w-3xl mx-auto p-4 space-y-4">
                ...
                <div className="rounded-lg border border-neutral-800 p-3">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {shuffledOptions[currentIdx].map(([code, text]) => {
                      ...
                      return (
                        <button key={`most-${code}`} onClick={()=>{
                            const next=[...answers]; if(next[currentIdx].least===code)return;
                            next[currentIdx]={...next[currentIdx], most: code as Trait}; setAnswers(next);
                          }}
                ...
                <div className="rounded-lg border border-neutral-800 p-3">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    {shuffledOptions[currentIdx].map(([code, text]) => {
                      ...
                      return (
                        <button key={`least-${code}`} onClick={()=>{
                            const next=[...answers]; if(next[currentIdx].most===code)return;
                            next[currentIdx]={...next[currentIdx], least: code as Trait}; setAnswers(next);
                          }}
                          style={{ textAlign:'left', color:'#fff', border:'1px solid '+(selected?KW_RED:'#3f3f46'), padding:'14px 16px', borderRadius:'12px', fontSize:'18px', background:selected?'#1c1c20':'#0f0f12' }}>{text}</button>
                      );
                    })}
        ...
        {/* PRINT/PDF REPORT (white, big fonts, KW red headings) */}
        <div ref={reportRef} style={{position:'absolute', left:'-9999px', top:'-9999px', width:'794px'}}>
          <div style={{background:'#fff', color:'#000', fontFamily:'ui-sans-serif, system-ui', padding:'28px', lineHeight:1.5}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between'}}>
              <img src={LOGO_SRC} alt="KW Explore" style={{height:'18px'}}/>
              <div style={{color:KW_RED, fontWeight:800}}>KW Explore</div>
            </div>

            <h1 style={{color:KW_RED, fontSize:'22px', fontWeight:800, margin:'16px 0 6px'}}>Personalized DISC Report</h1>
            <div style={{fontSize:'12px', marginBottom:'10px'}}>
              <b>Name:</b> {info.firstName} {info.lastName} &nbsp; <b>Email:</b> {info.email} &nbsp; <b>Phone:</b> {info.phone||'—'}
            </div>

            <h2 style={{color:'#111', fontSize:'16px', fontWeight:700, margin:'12px 0 6px'}}>Executive Summary</h2>
            <p style={{fontSize:'12px'}}>Your natural emphasis appears strongest in <b>{TRAIT_INFO[primary].label}</b>{secondary?` with a supporting ${TRAIT_INFO[secondary].label} blend`:''}. Your adaptive pattern suggests you lean on complementary behaviors when under pressure.</p>
            ...
            <h3 style={{fontWeight:700, fontSize:'14px', marginTop:'10px'}}>Natural vs Adaptive (Side‑by‑Side)</h3>
            ...
            <h2 style={{color:KW_RED, fontSize:'16px', fontWeight:800, margin:'16px 0 8px'}}>Lead Generation Action Plan</h2>
            <div style={{fontSize:'12px', marginBottom:'8px'}}><b>Focus:</b> {plan.title}</div>
            ...
            <h2 style={{color:KW_RED, fontSize:'16px', fontWeight:800, margin:'16px 0 8px'}}>Negotiation Playbook</h2>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px'}}>
              <div><h3 style={{fontWeight:700}}>Approach</h3><ul>{neg.approach.map((x,i)=>(<li key={i}>{x}</li>))}</ul><h3 style={{fontWeight:700, marginTop:'6px'}}>Tactics</h3><ul>{neg.tactics.map((x,i)=>(<li key={i}>{x}</li>))}</ul></div>
              <div><h3 style={{fontWeight:700}}>Watch‑outs</h3><ul>{neg.watchouts.map((x,i)=>(<li key={i}>{x}</li>))}</ul><h3 style={{fontWeight:700, marginTop:'6px'}}>Useful Phrases</h3><ul>{neg.phrases.map((x,i)=>(<li key={i}>{x}</li>))}</ul></div>
            </div>

            <h2 style={{color:KW_RED, fontSize:'16px', fontWeight:800, margin:'16px 0 8px'}}>Communicating with Each Style (Lead Gen & Negotiations)</h2>
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px'}}>
              <div><h3 style={{fontWeight:700}}>With D</h3><ul><li>Be brief, outcome‑first. Offer choices + deadlines.</li><li>Scripts: “Fastest path to X is Y. Does that work?”</li></ul></div>
              <div><h3 style={{fontWeight:700}}>With I</h3><ul><li>Warm tone. Social proof and future‑pacing.</li><li>Scripts: “Imagine move‑in day looks like … shall we book the next step?”</li></ul></div>
              <div><h3 style={{fontWeight:700}}>With S</h3><ul><li>Lower pressure. Clarify steps and remove risk.</li><li>Scripts: “Would it help if I handled X so you can do Y?”</li></ul></div>
              <div><h3 style={{fontWeight:700}}>With C</h3><ul><li>Lead with evidence and process. Share sources.</li><li>Scripts: “Based on the comps and absorption, the range is X–Y. What variables would change your view?”</li></ul></div>
            </div>

            <h2 style={{color:KW_RED, fontSize:'16px', fontWeight:800, margin:'16px 0 8px'}}>30‑60‑90 Implementation</h2>
            <ul style={{fontSize:'12px'}}><li><b>Next 30 days:</b> Execute the daily/weekly cadence. Track in CRM.</li><li><b>31–60 days:</b> Double down on what converts; templatize it.</li><li><b>61–90 days:</b> Add one new channel aligned to your style; ship one case study.</li></ul>

            <h2 style={{color:KW_RED, fontSize:'16px', fontWeight:800, margin:'16px 0 8px'}}>Scoreboard</h2>
